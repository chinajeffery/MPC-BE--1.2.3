/*
 * $Id: MainFrm.cpp 2933 2013-06-18 12:58:06Z szl $
 *
 * (C) 2003-2006 Gabest
...
void CMainFrame::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (!m_pFullscreenWnd->IsWindow() || !m_OSD.OnLButtonDown (nFlags, point)) {
		...
		if (GetPlaybackMode() == PM_DVD) {
			...
			if (SUCCEEDED(pDVDI->GetButtonAtPosition(pDVD, &pulButtonIndex))) {
				bDVDButtonAtPosition = true;
			}

			if (SUCCEEDED(pDVDC->ActivateAtPosition(pDVD))
...
void CMainFrame::SaveDIB(LPCTSTR fn, BYTE* pData, long size)
{
	...
	if (ext == _T(".bmp")) {
		...
		WebPDIB(fn, pData, s.iThumbQuality);
	} else if (ext == _T(".webpll")) {
		WebPDIB(fn, pData, 0);
	} else if (ext == _T(".tif")) {
		BMPDIB(fn, pData, L"image/tiff", s.iThumbQuality, 0, 0, 0);
	}
...
void CMainFrame::OnViewColorManagementEnable()
{
	...
	s.UpdateData(true);
	m_OSD.DisplayMessage(OSD_TOPRIGHT,
						 s.m_AdvRendSets.iVMR9ColorManagementEnable ? ResStr(IDS_OSD_RS_COLOR_MANAGEMENT_ON) : ResStr(IDS_OSD_RS_COLOR_MANAGEMENT_OFF));
}
	...
	if (m_iMediaLoadState == MLS_LOADED) {
		...
		if (GetPlaybackMode() == PM_FILE) {
			...
			if (m_iSpeedRate > 0) {
				...
				pDVDC->PlayBackwards(abs((double)m_iSpeedRate/10), DVD_CMD_FLAG_Block, NULL);
			}
			pDVDC->Pause(FALSE);
			pMC->Run();
		} else if (GetPlaybackMode() == PM_CAPTURE) {
	...
	if (GetPlaybackMode() == PM_CAPTURE) {
		...
		case 0:
			...
			if (GetPlaybackMode() == PM_FILE) {
				...
				hr = pMS->SetRate((double)iRate/10);
				...
				if (iRate > 0) {
					hr = pDVDC->PlayForwards(abs((double)iRate/10), DVD_CMD_FLAG_Block, NULL);
				} else {
					hr = pDVDC->PlayBackwards(abs((double)iRate/10), DVD_CMD_FLAG_Block, NULL);
				}
			}
...
void CMainFrame::OnUpdatePlayChangeRate(CCmdUI* pCmdUI)
{
	...
	if (m_iMediaLoadState == MLS_LOADED) {
		...
		if (!bAutoSpeedStep) {
			fEnable = true;
			if (GetPlaybackMode() == PM_FILE && (!fInc && m_iSpeedRate <= 2) || (fInc && m_iSpeedRate >= 120)) {
				fEnable = false;
			} else if (GetPlaybackMode() == PM_DVD && (!fInc && m_iSpeedRate <= -160) || (fInc && m_iSpeedRate >= 160)) {
				fEnable = false;
			} else if (GetPlaybackMode() == PM_DVD && m_iDVDDomain != DVD_DOMAIN_Title) {
				fEnable = false;
			} else if (m_fRealMediaGraph || m_fShockwaveGraph) {
...
void CMainFrame::MoveVideoWindow(bool fShowStats)
{
	if (m_iMediaLoadState == MLS_LOADED && !m_fAudioOnly && IsWindowVisible()) {
		...
		if (b_UseSmartSeek && m_wndView2) {
			...
			if (m_pMFVDC2) {
				m_pMFVDC2->SetVideoPosition (NULL, wr2);
			}
...
HRESULT CMainFrame::PreviewWindowShow(REFERENCE_TIME rtCur2)
{
	...
	if (GetPlaybackMode() == PM_DVD && pDVDC2) {
		DVD_PLAYBACK_LOCATION2 Loc, Loc2;
		double fps = 0;

		hr = pDVDI->GetCurrentLocation(&Loc);
		if (FAILED(hr)) {
			return hr;
		}

		hr = pDVDI2->GetCurrentLocation(&Loc2);
		...
		if (FAILED(hr) || (Loc.TitleNum != Loc2.TitleNum)) {
			...
			if (SUCCEEDED(hr)) {
				...
				hr = pDVDC2->PlayChapterInTitle(Loc.TitleNum, 1, DVD_CMD_FLAG_Block|DVD_CMD_FLAG_Flush, NULL);
				hr = pDVDC2->PlayAtTime(&dvdTo, DVD_CMD_FLAG_Flush, NULL);
				if (FAILED(hr)) {
...
void CMainFrame::OnNavMixStreamSelectSubMenu(UINT id, DWORD dwSelGroup)
{
	...
	if (GetPlaybackMode() == PM_FILE || (GetPlaybackMode() == PM_CAPTURE && AfxGetAppSettings().iDefaultCaptureDevice == 1)) {
		...
		pDVDC->SelectAudioStream(id, DVD_CMD_FLAG_Block, NULL);
		return;
	}
